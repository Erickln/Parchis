...
public class Node {
  ...
  // Tokens in the node
  protected ArrayList<Token> Tokens = new ArrayList<>(); // the quantity of tokens that can hold each node in the board (max=2)
  // Father of the node
  protected Node back;
  ...
  public Node(Node back, Node front, int pos, int playerOwner) {
    ...
    if (playerOwner <= 6 && playerOwner >= -1) {
      ...
      System.out.println(
        "Trying to create a node with a invalid playerOwner: " +
        playerOwner +
        "\nSafe in " +
  ...
  // This method will recieve a token and pop it from Tokens, else it will return
  // null
  ...
  public boolean canEnter(Token token, Hall hall) {
    if (token.getPlayerOwner() == hall.getPlayerOwner()) {
      return true;
    }
    return false;
  }
